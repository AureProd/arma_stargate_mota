...
_fond_bouton_achat ctrlSetBackgroundColor [0,0,0,0];
_fond_bouton_location ctrlSetBackgroundColor [0.6,0,0,0.25];
bouton_A_OK = true; // true = location / false = achat
...

{
	if (isMilitaire == (_x select 6)) then 
	{
		if ((_x select 5) <= (_playerBdd select 4)) then {
			...
			private _index = _liste_vehicules lbAdd format ["%1 (%2XP)", (_x select 1), (_x select 4)];
