...
_fond_bouton_munitions ctrlSetBackgroundColor [0.6,0,0,0.25];
_fond_bouton_viseurs ctrlSetBackgroundColor [0,0,0,0];
...

{
	if ((_x select 1) <= (_playerBdd select 4)) then 
	{
		...
		{
			private _picture_arme_id = getText(configFile >>  "CfgMagazines" >> (_x select 4) >> "picture");
