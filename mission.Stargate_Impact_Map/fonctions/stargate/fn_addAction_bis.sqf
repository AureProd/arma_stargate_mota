...

{
...

	{
		...
		if ((_x select 0) != (_planete select 0)) then {
			...
			}, format ["!((%1 getVariable ['isOpen', [false, nil, false, false, false]]) select 0) and !((%1 getVariable ['isOpen', [false, nil, false, false, false]]) select 3) and (alive _this) and ([%2, 0] call mission_fnc_has_planetes)", _porte, (_x select 0)], [_porte, _porteDistante]] call _addActions;
		};
