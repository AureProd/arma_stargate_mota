...
if (!(_gate getVariable ['is_open_gate', false]) and !(_remote_gate getVariable ['is_open_gate', false])) then {
	[_gate, _remote_gate] call sg_fnc_dhd; // run dhd controller function

	[_gate, _remote_gate] call mission_fnc_ring_bis; // TODO: function name

	sleep 0.3; // wait 3 miliseconds

	[_gate, ["gate_open", 50]] remoteExec ["say3D", 0]; // play sound in 3D for all players of server of the open_gate for local gate
	[_remote_gate, ["gate_open", 50]] remoteExec ["say3D", 0]; // play sound in 3D for all players of server of the open_gate for remote gate

	[_gate] remoteExec ["mission_fnc_kawoosh", 0]; // TODO: function name 
	[_remote_gate] remoteExec ["mission_fnc_kawoosh", 0]; // TODO: function name
