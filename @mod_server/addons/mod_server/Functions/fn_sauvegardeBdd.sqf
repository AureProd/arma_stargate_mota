...
// UID player / name player / classes [classe, race] / exp / licences / level / vie / faim / soif / inv reel / inv virtuel / position player / liste quetes faites / liste planetes visités / liste quetes actives / garage / white liste soldat
...
private _planete_visite = param[13];
private _quetes_actives = param [14];
private _garage = param [15];
private _whiteListeSoldat = param [16];
...
_documents = [_documents] call db_fnc_mresArray;
...
{
	private _query2 = format ["UPDATE player SET p_name = '%1', p_classe = %2, p_race = %3, p_exp = %4, p_licence = '%5', p_level = %6, p_vie = %7, p_faim = %8, p_soif = %9, p_inv_reel = '%10', p_inv_virtuel = '%11', p_position = '%12', p_quetes_faites = '%13', p_planete_visite = '%14', p_quetes_actives = '%15', p_garage = '%17', p_white_liste_soldat = %18, p_documents = '%19' WHERE p_uid = '%16'", _namePlayer, _classes select 0, _classes select 1, _exp, _licences, _level, _vie, _faim, _soif, _inv_reel, _inv_virtuel, _pos_player, _quetes_faites, _planete_visite, _quetes_actives, _steamUID, _garage, _whiteListeSoldat, _documents];
	[_query2, 1] call db_fnc_asyncCall;

	diag_log format ["Le joueur %1 à été synchronisé en bdd (UID : %2)", _namePlayer, _steamUID];
	// nom joueur / classe / race / exp / licences / level / vie / faim / soif / inv reel / inv virtuel / position player / liste quetes faites / liste planetes visités / liste quetes actives / garage / white liste soldat
};